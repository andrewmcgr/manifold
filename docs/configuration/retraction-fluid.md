...
- **Normal-Aligned Departure and Arrival Clearance**: Travel detours depart and arrive along CAD surface normals into open air using the configured `z_hop_height` for clearance length without redundant vertical Z lifts, while intermediate routing waypoints enforce $2 \times \text{line width}$ ($0.8\text{ mm}$) safety clearance from printed material.
