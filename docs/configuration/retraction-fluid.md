...
- **Travel Collision Avoidance (`travel_collision_avoidance_enabled`)**: Detects travel chords that cross solid material and plans collision-free detour paths in open air using a two-tier router (Tier 1 planar horizontal search with zero Z excursion, falling back to Tier 2 single clean trapezoidal flyover when obstructed).
...
- **Low-Angle Tangent Departure & Approach**: Travel detours depart and approach at a shallow angle ($\le 26^\circ$) along the local isosurface tangent plane rather than lifting steeply along surface normals. This keeps the capillary meniscus in pure shear to eliminate surface witness blobs and stringing, keeping the nozzle within outer walls during the wipe phase before easing outward into open air.
- **Planar-First Travel Transit**: When navigating around printed geometry in open air, Tier 1 routes horizontally at constant elevation $Z_{\text{travel}} = \max(\text{start}.z, \text{end}.z)$ with exactly $0.0\text{ mm}$ vertical excursion. If an obstacle blocks horizontal passage, Tier 2 executes a single trapezoidal flyover (one lift to peak height, horizontal cruise, and one descent), eliminating erratic multi-hop oscillations and staircasing.
