...
## 3. Temperatures
- **Default Nozzle Temp (°C)**: Base target hotend temperature.
- **Bed Temperature (°C)**: Target heated build plate temperature.
- **Chamber Temperature (°C)**: Target heated enclosure temperature ($0 = \text{unheated}$).

---

## 4. Wave Overhangs
- **Enable Wave Overhangs**: Activates 2D/3D Huygens wave propagation for support-free horizontal overhangs (LaSO).
- **Track Overlap (mm)**: Lateral overlap distance between adjacent wave passes (default $0.05\text{ mm}$).
- **Overhang Speed (mm/s)**: Printing speed for overhang moves (default $25\text{ mm/s}$).
...
- **Use Dynamic Fluid Model**: Toggles the 2-point power-law non-Newtonian pressure advance, extrudate swell, and adaptive fluid retraction engine.
  - *Fluid Dynamics Parameters*:
    ...
    - **Static Break Distance (mm)**: Mechanical pull distance to snap the capillary droplet bridge.
    - **Max Fan Temp Drop (°C)**: Maximum tip cooling drop under 100% fan.
    - **Ooze Time Constant $\tau$ (s)**: Asymptotic ooze relaxation rate.
    - **Max Ooze Prime (mm)**: Maximum extra filament re-primed after long travels.
    - **Swell Ratio @ Low Flow ($B_{\text{low}}$)**: Extrudate die swell expansion ratio under low shear.
    - **Swell Ratio @ High Flow ($B_{\text{high}}$)**: Extrudate die swell expansion ratio under high shear.
- *Standard Retraction Parameters* (when fluid dynamics is disabled):
  - **Retraction Distance & Speed**: Traditional retraction parameters.
...
- **Corner Overlap & SCV Flow Compensation**: Deducts redundant volume deposited on the inside of corners and along shortcutting Klipper SCV arcs to prevent inside corner bulging and ridges.
  - **Corner Compensation Ratio**: Sensitivity multiplier (default $1.00 = 100\%$ deduction).
- **Transient Nozzle Pressure Compensation**: Tracks accumulated melt zone pressure across consecutive moves using the hotend elasticity constant ($K = C_{\text{PA}}$) and dynamically scales down commanded volume ($M = (Q_{\text{target}} / P_{\text{average}})^\beta$) on rapid short-stroke infill and gaps.
  - **Min Flow Multiplier ($M_{\text{min}}$)**: Safety floor to prevent starvation (default $0.75$).
  - **Sensitivity Exponent ($\beta$)**: Power-law exponent scaling the flow reduction ratio (default $1.00$). Values $< 1.0$ attenuate the reduction; values $> 1.0$ make it more aggressive.
...
- **Z-hop Height (mm)**: Lift clearance.
...
- **Z Travel Penalty**: Cost weighting factor applied to vertical movements during travel path optimization and obstacle routing. Dynamically scales with the machine's Z vs. XY axis speed and acceleration ratio.
- **Simplify Wall Toolpaths**: Ramer-Douglas-Peucker simplification on curved wall loops.
- **Simplification Tolerance (mm)**: Vertex reduction tolerance.
...
- **Infill Line Width (mm)**: Extrusion width for infill scanlines.
- **Infill Angle (deg)**: Base rotation angle for rectilinear infills ($0\text{--}360^\circ$).
