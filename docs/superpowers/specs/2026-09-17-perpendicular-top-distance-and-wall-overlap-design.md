...
- **Plan F (fix the extrusion-volume-audit's sparse-infill expected-volume model)** — the `volume_audit` module shipped by the `2026-09-18-extrusion-volume-audit` plan computes expected sparse-infill volume as a single center-sample of `infill_density * cell_size^3`, which mismatches real sliced geometry badly enough (measured ~1.46x at `cell_size = 2.0`, ~5.94x at `cell_size = 0.8`, with 3629/14558 healthy non-empty cells already exceeding 2.0x at `cell_size = 0.8` on a plain 20mm box) that this modeling error swamps any real wall-shell duplication signal in the tool's global `assert_no_overfill` assertion: measured identical-to-13-significant-figures global max overfill ratio whether 0, 1, 95, or 193 inner-wall paths were duplicated on real pipeline output, at every `cell_size` tried. This means the tool's headline whole-grid assertion cannot currently catch wall-shell duplication — its original motivating bug class — on real prints; only the per-cell `overfilled_cells` query can (proven by a dedicated test), and only when a caller inspects it directly rather than relying on `assert_no_overfill`. The limitation is measured and documented in `audit_extrusion_volume`'s own doc comment and pinned by a test, but not fixed. Candidate approaches: derive expected sparse-infill volume from the pattern's actual line spacing/geometry (e.g. sampling multiple points per cell and estimating real infill coverage) rather than nominal density; or exclude/flag sparse-infill cells from the global ratio and report them through a separate, density-aware check. Should get its own design/plan cycle since it changes the central expected-volume model Task 1 established, not a bolt-on fix.
- **Plan G (fix the `mesh_sdf.rs` barycentric-weight swap)** — `manifold-fidget/src/mesh_sdf.rs:441-442`'s `edge_containing_point` assigns the Ericson RTCD `v`/`w` barycentric weights to the wrong output variables relative to the standard formula, discovered while implementing the `2026-09-18-extrusion-volume-audit` plan's Task 1 (its own test's query point had to be moved off a mesh seam to avoid the resulting wrong-edge feature-normal lookups and wrong-sign SDF values near edges). Real, precisely diagnosed, pre-existing defect in a different crate; left unfixed as out of scope for that plan. Likely affects any SDF query near a mesh edge/seam, not just the volume-audit tool — worth investigating whether it explains any other edge-adjacent slicing artifacts before scoping a fix.

- **Plan E (test suite performance, tooling)** — the workspace test suite (`cargo test --workspace`) is extremely slow and compute-intensive, dominated by repeated full mesh-to-layers slicing across many tests. Refactor the test suite around a "slice once, probe many properties" pattern wherever possible: build a small, shared set of representative test objects (ideally minimizing the total count, choosing geometries that jointly exercise the properties currently tested), slice each one exactly once per relevant `SlicerConfig`, and have individual test functions assert against the already-computed `Layer`/`Path` results rather than each independently re-slicing its own one-off mesh. This session's own plan execution repeatedly hit 30-minute subagent dispatch timeouts specifically on `cargo test --workspace`, and several individual tests in `slicing.rs` already build near-duplicate cone/box/frustum fixtures (see the already-ledgered `cone_mesh_at`/`frustum_mesh_at`/inline-cone duplication noted during this plan's own review) — concrete evidence the current test structure is paying for the same slice repeatedly. Not designed yet — open questions include how to keep tests independently readable/debuggable when they share a slice result, how to avoid one test's assertion coupling to another's expectations, and whether the shared-fixture slices should be computed once per test binary run (e.g. via `std::sync::OnceLock`) or restructured into fewer, denser test functions each asserting multiple properties.
- **Plan D (tooling, not a slicing fix) — DONE, implemented as `2026-09-18-extrusion-volume-audit`.** Shipped a raw-SDF-grounded volume audit tool (`crates/manifold-core/src/volume_audit.rs`) that accumulates toolpath extrusion into a global 3D voxel grid and checks it against an independently-derived expected volume, catching wall/solid-fill duplication and empty voids. See Plan F above for the one significant residual gap found while shipping it (sparse-infill expected-model mismatch swamps the global assertion's ability to catch wall-shell duplication on real prints).
