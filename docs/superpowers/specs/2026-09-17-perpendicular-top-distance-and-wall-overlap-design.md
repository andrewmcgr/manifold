...
- **Plan D (tooling, not a slicing fix)** — no way currently exists to measure, in code, how much extrusion actually lands where. Track extrusion volume by accumulating each toolpath segment's extruded volume into a moderately coarse 3D grid (a GPU compute pass is a natural fit, given the segment count on a real print), so the accumulated volume per cell can be visualized directly and, more importantly, checked programmatically against the mesh's own expected solid/void volume in that cell — catching both near-duplicate extrusion (accumulated volume per unit cell exceeding what a single solid fill would produce, observed anecdotally at ratios above 2x in some prints near small top surfaces) and empty voids (cells that should be solid, e.g. near the bed, but never got extruded into at all). Motivated directly by this plan's own wall/solid-fill duplication bug (Tasks 3/4) and the plan's own known residual gaps (Plan B/C above): a volume-accuracy check like this would make exactly this class of bug detectable by an automated test rather than requiring a human review to notice a physically wrong-looking print. Not designed yet — open questions include grid resolution/adaptivity, whether to run this as a slicer-internal diagnostic, a test-only tool, or both, and how it interacts with intentionally-multi-pass regions (e.g. deliberate over-extrusion at seams).
