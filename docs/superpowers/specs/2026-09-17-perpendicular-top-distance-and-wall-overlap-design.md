...
- **Plan F (fix the extrusion-volume-audit's sparse-infill expected-volume model) — DONE, implemented as `2026-09-18-infill-aware-volume-audit`.** Classified every grid cell into `FillZone::{Outside, Solid, SparseInfill}` and restricted `overfilled_cells`/`underfilled_cells`/`assert_no_overfill`/`assert_no_underfill` to `Solid`-zone cells only, giving sparse infill its own separate, deliberately coarser whole-grid aggregate check (`infill_aggregate_ratio`/`assert_infill_volume_within`) instead of per-cell precision. `assert_no_overfill` now genuinely catches wall-shell duplication on real pipeline output, independently re-verified live during the plan's final review. The `volume_audit` module shipped by the `2026-09-18-extrusion-volume-audit` plan originally computed expected sparse-infill volume as a single center-sample of `infill_density * cell_size^3`, which mismatched real sliced geometry badly enough (measured ~1.46x at `cell_size = 2.0`, ~5.94x at `cell_size = 0.8`) that this modeling error swamped any real wall-shell duplication signal in the tool's global `assert_no_overfill` assertion.
- **Plan G (fix the `mesh_sdf.rs` barycentric-weight swap) — DONE, merged `0da67ab`.** `manifold-fidget/src/mesh_sdf.rs`'s `edge_containing_point` assigned the Ericson RTCD `v`/`w` barycentric weights to the wrong output variables relative to the standard formula, discovered while implementing the `2026-09-18-extrusion-volume-audit` plan's Task 1. Fixed directly via TDD (small, precisely diagnosed fix, full plan process skipped by agreement): added a scalene-triangle regression test that failed on the buggy code, swapped the `bary_b`/`bary_c` RHS assignments.
