...
//! mesh vertex whose order value falls inside that layer's order band
//! `[layer.order, layer.order + layer_height)` has *some* wall-0 loop
//! point within a small distance. A cluster of surface vertices in a
//! layer's band with no nearby wall-0 geometry means the external wall
//! is missing there.
//!
...
fn main() -> anyhow::Result<()> {
    ...
    // Area-weighted vertex normals, for plateau detection.
    let mesh = &objects[0].mesh;
    let mut vertex_normals = vec![DVec3::ZERO; vertices.len()];
    ...
    // A vertex is on a plateau face when its normal is nearly parallel
    // (either sign) to the order-field gradient: base/top faces covered
    // by solid fill rather than wall-0 loops.
    let is_plateau = |field: &dyn manifold_fidget::order::OrderField, v: DVec3, i: usize| {
        ...
        n.normalize().dot(g.normalize()).abs() > 0.5
    };
    ...
    for layer in &layers {
        ...
        for (i, &v) in vertices.iter().enumerate() {
            ...
            if is_plateau(field.as_ref(), v, i) {
                continue;
            }
            in_band += 1;
        ...
        if uncovered > 0 {
            ...
            println!(
                "layer {:3} (order {:8.3}): {}/{} band vertices uncovered, worst {:.2} mm at ({:.2}, {:.2}, {:.2}); wall0 pts={}",
